...
final class ClipboardHistoryState: ObservableObject {
    ...
    init(pasteboard: PasteboardClient = SystemPasteboardClient(), persistence: ClipboardHistoryPersistence? = ClipboardHistoryPersistence(), configuration: ClipboardConfig = .default) {
        self.pasteboard = pasteboard; self.persistence = persistence; lastChangeCount = pasteboard.changeCount
    ...
    var visibleItems: [ClipboardHistoryItem] { let q = query.trimmingCharacters(in: .whitespacesAndNewlines).lowercased(); return q.isEmpty ? items : items.filter { $0.title.lowercased().contains(q) || $0.subtitle.lowercased().contains(q) || $0.kindLabel.lowercased().contains(q) } }
    ...
    private func persist() { guard let persistence else { return }; do { try persistence.save(items) } catch let persistenceError { error = persistenceError } }
