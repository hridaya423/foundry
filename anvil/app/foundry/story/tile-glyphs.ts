export const glyphs: Record<string, number[]> = {
  ...
  "×": [0,17,10,4,10,17,0], "%": [25,25,2,4,8,19,19],
  ".": [0,0,0,0,0,0,4], ",": [0,0,0,0,0,4,8],
  ":": [0,4,4,0,4,4,0], ";": [0,4,4,0,4,4,8],
  "’": [4,4,8,0,0,0,0], "-": [0,0,0,31,0,0,0],
};

export function tileText(text: string) {
  ...
  lines.forEach((line, row) => [...line].forEach((character, column) => {
    ...
    glyph.forEach((bits, y) => {
      for (let x = 0; x < 5; x++) {
        if (bits & (1 << (4 - x))) points.push({ x: column * 6 + x - (line.length * 6 - 2) / 2, y: row * 10 + y - (lines.length * 10 - 4) / 2 });
      }
    });
  }));
  return { points, width, height: lines.length * 10 - 3 };
}
